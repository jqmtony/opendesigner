...
public class panelOpenOO5 extends PanelOpenBase implements IPanelOpenBase
{
	private static String sqlQuery = "SELECT Gname,GRef,GLevel,COALESCE(GVersionNumber,'1.0'),GDesignDate,GUpdateDate,ProjectID,GVisPublic,COALESCE(GAuthor,''),GId,GOrientation FROM tblgraphs WHERE GLevel = 4  AND ProjectID=";

    /**
	 * This is the default constructor
	 */
	public panelOpenOO5(ooOpenGenericDlg parent) {
		super();
	...
	private void initialize() {
		...
		add(gui.getLblTit("Documentos", 0, 0, 600, 30), null);
	...
	private JComponent getLbl0(String label, int xx, int yy, int width,int height) {
	...
	private JComponent getLbl1(String label, int xx, int yy, int width,int height) {
	...
	private JComponent getLbl4(String label, int xx, int yy, int width,int height) {
		lbl4 = new JLabel(label);
		lbl4.setBounds(new Rectangle(xx, yy, width, height)); //x,y,width,height
		return lbl4;
	}

	private JComponent getLbl5(String label, int xx, int yy, int width,int height) {
