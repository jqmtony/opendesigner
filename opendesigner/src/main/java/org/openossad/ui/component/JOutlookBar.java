...
public class JOutlookBar extends JPanel implements ActionListener
{
   ...
   * @param mouseListener
   */
  public JOutlookBar(MouseListener mouseListener)
  {
    ...
    public JOutlookBar()
    {
        new JOutlookBar(null);
    }

    public void addTab(String title, String[] buttons) {
		 ...
		 * top.setOpaque(true);
		 * top.setBackground(panel.getBackground().brighter()); panel.add(top);
		 */

		for (int i = 0, c = buttons.length; i < c; i += 2) {
			JButton button = new JButton(buttons[i].trim());
...
//				e.printStackTrace();
//			}
			button.setIcon(ImageUtil.createImageIcon(buttons[i + 1]));
			button.setFont(new Font(this.getFont().getName(), Font.BOLD, 11));
			panel.setBackground(new Color(144, 153, 174));
			panel.add(button);
		}

        JScrollPane scroll = new JScrollPane(panel);
        this.addBar(title, scroll);
